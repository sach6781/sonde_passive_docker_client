...
class DockerClient extends React.Component {
    ...
    checkEnrollmentStatus = (user_var) => {
        fetch('https://teams.dev.sondeservices.com/user/' + user_var + '/enrollment')
            .then(response => response.json())
            .then(data => {
                console.log(data)
                this.setState({ enrollmentStatus: data.enrolled, userId: user_var, showEnrollment: true })
            });
    ...
    enrollUser = () => {
        ...
        fetch('https://teams.dev.sondeservices.com/user/' + this.state.userId + '/enrollment', requestOptions)
            .then(response => response.json())
            .then(result => {
    ...
    uploadToS3 = () => {
        console.log("Going to get features for user-id - ", this.state.userIdentifier)
        const formData = new FormData();
        formData.append("webmasterfile", this.state.blobData);
        var requestOptions = {
            method: 'POST',
        ...
        fetch('https://teams.dev.sondeservices.com/user/' + this.state.userIdentifier + '/voice-features', requestOptions)
            .then(response => response.json())
            .then(result => {
    ...
    clickMe = (event) => {
        this.setState({
            historyOn: false, showRedError: false
        })
        const id = event.currentTarget.id;
        const id_array = id.split('_');
        console.log(id, ' - Got the Id')
        this.setState({
            tempData: this.state.historyData[id_array[1]][id_array[0]], historyOn: true, aggScore: this.state.mappingData[id_array[1]].score, dateToday: this.state.mappingData[id_array[1]].inferred_at, currentSessionCount: this.state.mappingData[id_array[1]].name
        })
    }
    render() {
        return (<div style={{ backgroundColor: "#00344E" }}>
            <div style={{ backgroundColor: "#00344E", width: "100%", height: "100%", }}>

                <div hidden={this.state.isRecording || this.state.finalScore.isLoading}>
                    <div hidden={this.state.finalScore.isData} style={{ border: "1.5px solid #30A7FF", position: 'absolute', top: '21%', right: "25%", width: "50%", height: "40%", backgroundColor: "#00344E", borderRadius: "15px", padding: "13px", color: "#b2dfee" }}>
                        <h1>
                            Welcome to Sonde Mental Fitness Tracker App for TEAMS! We'll analyze ONLY your voice and share the scores ONLY with you.
                        </h1></div>
                </div>
            </div>
