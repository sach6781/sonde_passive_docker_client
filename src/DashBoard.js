...
class DashBoard extends React.Component {
  ...
  fetchData = () => {
    ...
    const promises = this.state.names.map((name) =>
      fetch(
        'https://teams.dev.sondeservices.com/api/user-management/user/' +
          name +
          '/chunks'
      )
  ...
  getUserScoreHistory = () => {
    fetch(
      'https://teams.dev.sondeservices.com/api/user-management/users-history'
    )
      .then((response) => response.json())
      .then((result) => {
        const data = result
        ...
        fetch('https://teams.dev.sondeservices.com/api/user-management/users')
          ...
          .then((result) => {
            ...
            const all_enrolled_users = updatedData.map(
              (item) => item.identifier
            )
            const missingIdentifiers = all_enrolled_users.filter(
              (id) => !users_with_history.includes(id)
            )
      ...
      .then((result) => {
        ...
        fetch('https://teams.dev.sondeservices.com/api/user-management/users')
          ...
          .then((result) => {
            ...
            this.setState({
              names: all_enrolled_users,
              user_color_map: userColors
            })
            console.log('Setting the users score - ', data)
            this.setState({ userHistory: data })
          })
  ...
  generateVoiceFeatures = (recorded_data) => {
    ...
    console.log(
      'Invoking server to get the voice feature response for recorded_data - ',
      recorded_data
    )
    ...
    fetch(
      'https://teams.dev.sondeservices.com/docker-voice-features',
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        ...
        if (
          ...
          result.length > 0 &&
          result[0].hasOwnProperty('chunks') &&
          result[0].chunks === 1
        ) {
          this.setState({
            unverified: false,
            verified_user: result[0].user_identifier + "'s voice"
          })
          this.refs.child.addBox(
            this.state.user_color_map[result[0].user_identifier]
          )
        }
    ...
    return (
      <div style={{ height: '400px' }}>
        ...
        <ScoreSlider
          ...
          name_chunks_map={this.state.chunksMap}
          user_color_map={this.state.user_color_map}
        />
