...
class MentalFitness extends React.Component {
  ...
  getRemainingChunks = (name) => {
    fetch(
      "https://teams.dev.sondeservices.com/api/user-management/user/Guests/chunks",
    )
  ...
  generateVoiceFeatures = (recorded_data) => {
    ...
    fetch(
      "https://teams.dev.sondeservices.com/docker-voice-features?identifier=Guests",
      requestOptions,
    )
      .then((response) => response.json())
      .then((result) => {
        console.log(
          "Got the response from server for voice features - ",
          result,
        );
        ...
        if (
          ...
          this.setState({
            statusToShow: "Voice detected",
            verified_user: result[0].user_identifier,
          });
  ...
  fetchAgentHistory = () => {
    fetch(
      "https://teams.dev.sondeservices.com/api/user-management/user/Guests/scoring-history",
    )
      .then((response) => response.json())
      .then((result) => {
    ...
    return (
      <div>
        ...
        <div style={{ bottom: "0%" }} hidden={!this.state.isRecording}>
          ...
          <h5>30 sec. voice sample needed</h5>
        </div>
        <div hidden={!this.state.isRecording}>
          <h3>{this.state.statusToShow}</h3>
        </div>
