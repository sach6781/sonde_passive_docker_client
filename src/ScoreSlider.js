...
class ScoreSlider extends Component {
  ...
  getSubScores = (score_id, score, timestamp) => {
    fetch(
      'https://teams.dev.sondeservices.com/api/user-management/score/' +
        score_id
    )
      .then((response) => response.json())
      .then((result) => {
        ...
        this.setState({
          subScores: result.voiceFeatures,
          showPopup: true,
          final_score: score,
          ...
          {Object.keys(test_data).map((name) => (
            <div key={name}>
              ...
              >
                ...
                {chunk_map !== 10000
                  ...
                  <h style={{ fontSize: '12px' }}>
                    {' '}
                    (
                    {chunk_map[name] !== undefined && chunk_map[name] !== null
                      ? chunk_map[name]
                      : 0}
                    /30 sec.){' '}
                  </h>
              ...
              {test_data[name].map((data, index) => (
                <span
                  onClick={() =>
                    this.openPopup(data.id, data.calculated_at, data.score)
              ...
              {Array.from(
                { length: 10 - test_data[name].length },
                (_, index) => (
                  <span
                    key={index}
                    style={{
        ...
        <div>
          ...
          {this.state.showPopup && (
            <div style={popupStyle}>
              ...
              <div style={popupContentStyle}>
                <ShowSubScores
                  var={this.state.subScores}
                  final_score={this.state.final_score}
                  time_stamp={this.state.final_timestamp}
                >
                  {' '}
                </ShowSubScores>
              </div>
          ...
          {this.state.showPopUpChunks && (
            <div style={popupStyle}>
              <button
                style={closeButtonStyle}
                onClick={this.handleContinueClickChunks}
              >
                <div className="cross" style={crossStyle}>
                  <div
                    className="line line1"
              ...
              <div style={popupContentStyle}>
                <h3>Segments analyzed - {this.state.remainingChunks}</h3>
              </div>
            </div>
          )}
        </div>
